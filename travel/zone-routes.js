(function(){
  ...
  const routeCard=(zone)=>{
    ...
    if(!d)return `<article class="trust-card route-card route-unmapped" data-zone="${esc(zone.toLowerCase())}"><small>DESTINATION</small><h3>${esc(zone)}</h3><p>Listed in the Current Reality world directory. A route card appears here only after its walkthrough is written into the Compendium.</p></article>`;
    const routes=(d.routes||[]).map((r,i)=>`<details class="route-detail"><summary>${esc(r.name||`Route ${String.fromCharCode(65+i)}`)}</summary>${routeDiagram(r,i)}<ol class="guide-steps">${(r.steps||[]).map(s=>`<li>${esc(s)}</li>`).join('')}</ol></details>`).join('');
